...
@section('container')
    ...
    <script>
      document.addEventListener("DOMContentLoaded", () => {
        const username = @json(auth()->user()->username ?? 'User');

        // Cek dulu apakah elemen pesan sudah ada
        if (!document.getElementById('welcome-message')) {
          const message = document.createElement("div");
          message.id = "welcome-message"; // beri ID agar bisa dicek selanjutnya
          message.innerText = `👋 Hai ${username}, selamat datang!`;
